...
export async function GET(request: NextRequest) {
  ...
  try { return NextResponse.json(await runDiscoveryJob()); } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : "Synchronisatie mislukt" }, { status: 500 }); }
}
