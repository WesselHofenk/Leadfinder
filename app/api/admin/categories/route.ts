...
const patchSchema = z.object({
  ...
  priority: z.coerce.number().int().min(1).max(999).optional(),
}).refine((input) => input.isActive !== undefined || input.priority !== undefined);
const slug = (value: string) => value.toLowerCase().normalize("NFKD").replace(/[\u0300-\u036f]/g, "")
...
export async function POST(request: NextRequest) {
  if (!await authorized()) return NextResponse.json({ error: "Niet toegestaan" }, { status: 403 });
  ...
  if (input.data.kind === "category") {
    ...
    await prisma.excludedCategory.create({ data: {
      name: input.data.name,
      slug: slug(input.data.name),
      reason: input.data.reason || "Handmatig uitgesloten",
    } });
  }
