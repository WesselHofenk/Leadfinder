...
export default async function DashboardPage() {
  const active: Prisma.LeadWhereInput = { isActive: true, isFiltered: false, isSuppressed: false, businessStatus: { in: ["OPERATIONAL", "UNKNOWN"] }, phoneNumber: { not: "" }, ...googleVerifiedNoWebsiteWhere };
  ...
  return <div className="content">
    <header className="page-head"><div><span className="eyebrow">Overzicht</span><h1>Dashboard</h1><p className="muted">Alleen door Google geverifieerde bedrijven zonder eigen website.</p></div><div className="actions"><GenerationButton/><Link className="button button-secondary" href="/leads">Bekijk alle leads <ArrowRight size={15}/></Link></div></header>
    <section className="stats"><Stat icon={<Database size={18}/>} label="Actieve leads" value={total} meta={`${today} vandaag`}/><Stat icon={<Globe2 size={18}/>} label="Geen website" value={noWebsite} meta="Door Google bevestigd"/><Stat icon={<TrendingUp size={18}/>} label="Google geverifieerd" value={websiteOpportunities} meta={`Gem. score ${Math.round(average._avg?.opportunityScore ?? 0)}`}/><Stat icon={<MapPinned size={18}/>} label="Gefilterd" value={filtered} meta="Aparte pipeline"/><Stat label="Gebeld" value={called} meta="Opvolging"/><Stat label="Geen gehoor" value={noAnswer} meta="Blijft bewaard"/><Stat label="Offerte gestuurd" value={quoted} meta="Commerciële kans"/><Stat label="Gefactureerd" value={invoiced} meta="Gewonnen"/></section>
