...
vi.mock("server-only", () => ({}));
const { resetCandidates, resetRun, findExpiredRuns } = vi.hoisted(() => ({ resetCandidates: vi.fn(), resetRun: vi.fn(), findExpiredRuns: vi.fn() }));
vi.mock("@/lib/prisma", () => ({
  prisma: {
    generationCandidate: { updateMany: resetCandidates },
    generationRun: { findMany: findExpiredRuns, updateMany: resetRun },
  },
}));
...
describe("herstel van een onderbroken generatiebatch", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    findExpiredRuns.mockResolvedValue([]);
    resetCandidates.mockResolvedValue({ count: 2 });
    resetRun.mockResolvedValue({ count: 1 });
  });
