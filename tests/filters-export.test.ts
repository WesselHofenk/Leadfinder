...
describe("filters en paginering",()=>{
  ...
  it("filtert Geen eigen website uitsluitend op de centrale websiteStatus",()=>{expect(activeLeadWhere(parseLeadFilters({websiteStatus:"NO_OWN_WEBSITE"}))).toMatchObject({websiteStatus:"NO_OWN_WEBSITE"});expect(activeLeadWhere(parseLeadFilters({leadType:"NO_WEBSITE"}))).toMatchObject({websiteStatus:"NO_OWN_WEBSITE"})});
  it("past de Geen eigen website-filter toe voordat pagina 2 wordt opgehaald",()=>expect(buildLeadListQuery(parseLeadFilters({websiteStatus:"NO_OWN_WEBSITE",page:"2",pageSize:"25"}))).toMatchObject({where:{websiteStatus:"NO_OWN_WEBSITE"},skip:25,take:25}));
});
