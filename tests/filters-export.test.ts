...
describe("filters en paginering",()=>{
  ...
  it("toont automatisch gevalideerde geen-websiteleads direct in de actieve lijst",()=>{const where=activeLeadWhere(parseLeadFilters({}));expect(where).toMatchObject({websiteStatus:"NO_WEBSITE_CONFIRMED"});expect(where).not.toHaveProperty("googleWebsiteVerifiedAt")});
