...
describe("filters en paginering",()=>{
  ...
  it("verbergt onbevestigde leads standaard en toont ze in de controlepipeline",()=>{expect(activeLeadWhere(parseLeadFilters({}))).toMatchObject({isActive:true,isFiltered:false,websiteStatus:"NO_WEBSITE_CONFIRMED"});expect(activeLeadWhere(parseLeadFilters({filtered:"yes"}))).toMatchObject({isFiltered:true})});
  it("filtert geen website uitsluitend op status plus actuele Google-bevestiging",()=>expect(activeLeadWhere(parseLeadFilters({websiteStatus:"NO_WEBSITE_CONFIRMED"}))).toMatchObject({websiteStatus:"NO_WEBSITE_CONFIRMED",googleWebsitePresent:false,googleWebsiteVerifiedAt:{not:null}}));
  it("toont standaard uitsluitend handmatig bevestigde geen-websiteleads",()=>expect(activeLeadWhere(parseLeadFilters({}))).toMatchObject({websiteStatus:"NO_WEBSITE_CONFIRMED",googleWebsitePresent:false,googleWebsiteVerifiedAt:{not:null}}));
