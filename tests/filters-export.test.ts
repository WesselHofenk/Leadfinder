...
describe("filters en paginering",()=>{
  ...
  it("bouwt vaste leadvoorwaarden en filters",()=>expect(activeLeadWhere(parseLeadFilters({country:"BE",city:"Gent",leadType:"NO_WEBSITE",minConfidence:"70"}))).toMatchObject({isActive:true,isFiltered:false,isSuppressed:false,businessStatus:{in:["OPERATIONAL","UNKNOWN"]},country:"BE",websiteStatus:"NO_OWN_WEBSITE",confidenceScore:{gte:70}}));
  ...
  it("vereist naast no_website ook expliciet Google-bewijs",()=>{const proof={websiteStatus:"NO_OWN_WEBSITE",googleWebsitePresent:false,googleWebsiteVerifiedAt:{not:null},googlePlaceId:{not:null}};expect(activeLeadWhere(parseLeadFilters({websiteStatus:"NO_OWN_WEBSITE"}))).toMatchObject(proof);expect(activeLeadWhere(parseLeadFilters({leadType:"NO_WEBSITE"}))).toMatchObject(proof)});
  it("toont standaard uitsluitend Google-geverifieerde bedrijven zonder website",()=>expect(activeLeadWhere(parseLeadFilters({}))).toMatchObject({websiteStatus:"NO_OWN_WEBSITE",googleWebsitePresent:false,googleWebsiteVerifiedAt:{not:null},googlePlaceId:{not:null}}));
  it("past de Google-bewijsfilter toe voordat pagina 2 wordt opgehaald",()=>expect(buildLeadListQuery(parseLeadFilters({websiteStatus:"NO_OWN_WEBSITE",page:"2",pageSize:"25"}))).toMatchObject({where:{websiteStatus:"NO_OWN_WEBSITE",googleWebsitePresent:false,googleWebsiteVerifiedAt:{not:null},googlePlaceId:{not:null}},skip:25,take:25}));
});
