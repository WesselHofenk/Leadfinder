...
describe("gestructureerde generatie-API", () => {
  it("geeft alle publieke tellers met stabiele veldnamen terug", () => {
    const run = {
      ...
      processedSegments: 18, sourceFailures: 3, sourceRequests: 31, sourceSuccesses: 20,
      sourceEmptyResponses: 8, sourceTechnicalErrors: 11,
      consecutiveSourceFailures: 1, progress: 100, stopReason: "7 geldige leads opgeslagen.",
    };
    expect(generationResponse(run)).toEqual(expect.objectContaining({
      ...
      rejectedInvalid: 5, failedQueries: 3, logicalSegments: 21, successfulSegments: 18,
      sourceRequests: 31, sourceSuccesses: 20, sourceEmptyResponses: 8, sourceTechnicalErrors: 11,
      consecutiveFailedQueries: 1, progress: 100, message: "7 geldige leads opgeslagen.", run,
    }));
  });
