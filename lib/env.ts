...
const schema = z.object({
  ...
  GENERATION_MAX_CANDIDATES: z.coerce.number().int().min(100).max(1000).default(1000),
  ...
  GENERATION_BATCH_CANDIDATES: z.coerce.number().int().min(5).max(50).default(25),
  GENERATION_BATCH_WEBSITE_CHECKS: z.coerce.number().int().min(1).max(10).default(8),
